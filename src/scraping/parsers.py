...
def rabota(url, language=None, city=None):
    jobs = []
    errors = []
...
def dou(url, language=None, city=None):
    jobs = []
    errors = []
    # domain = 'https://www.work.ua'
    if url:
...
def djinni(url, language=None, city=None):
    jobs = []
    errors = []
    ...
    if url:
        ...
        if resp.status_code == 200:
            soup = BS(resp.content, 'html.parser')
            main_section = soup.find('section', class_='jobs-list-wrapper')
            if main_section:
                article_list = main_section.find_all('article', attrs={'class': 'svelte-re68m3'})
                for article in article_list:
                    ...
                    content = cont[2].text
                    company = 'No name'
                    a = title.find('span', attrs={'class': 'company'})
                    if a:
                        company = a.text
                    jobs.append({'title': title.text, 'url': domain + href,
                                 'description': content, 'company': company,
                ...
                errors.append({'url': url, 'title': "Article does not exists"})
        else:
            errors.append({'url': url, 'title': "Page do not response"})
